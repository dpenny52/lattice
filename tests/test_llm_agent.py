...
class TestComputeMood:
    """Tests for the _compute_mood() method."""

    def test_default_mood_is_sleeping(
        self, router: Router, recorder: SessionRecorder
    ) -> None:
        """New agent starts idle — mood is 😴."""
        provider = MockProvider([])
        agent = _make_agent(router, recorder, provider)
    ...
    async def test_mood_thinking_during_processing(
        self, router: Router, recorder: SessionRecorder
    ) -> None:
        """While processing a message, mood should be 🤔."""
        observed_mood: list[str] = []

        # Custom provider that captures mood mid-call.
        class SpyProvider:
            async def chat(
                self,
                messages: list[dict[str, Any]],
    ...
    async def test_mood_idle_after_text_response(
        ...
        # After handle_message completes, agent goes back to idle.
        assert agent._state == "idle"
        assert agent._compute_mood() == "\U0001f634"  # 😴

    async def test_mood_error_after_retries_exhausted(
        self, router: Router, recorder: SessionRecorder
    ) -> None:
        """After all retries fail, mood is 😡."""
        provider = AlwaysFailProvider()
        agent = _make_agent(router, recorder, provider)
        router.register("agent-a", agent)
        ...
        # Error state persists — not reset to idle.
        assert agent._state == "error"
        assert agent._compute_mood() == "\U0001f621"  # 😡

    def test_mood_unknown_state_defaults_sleeping(
        self, router: Router, recorder: SessionRecorder
    ) -> None:
